...
module.exports = {
    async login(req, res) {
        ...
        const { acess_id, password } = req.body;
        const Secretary = await SecretaryModel.find(acess_id);
        if (!Secretary) return res.status(400).json({ error: 'Invalid id or password' })
        ...
        return res.json({ token, type: 'Bearer', user: {id: Secretary.id, acess_id: Secretary.acess_id, name: 'Secretaria Municipal de Saúde'} })
    },

    async resetPassword(req, res) {
        const { secretary_id } = req.headers;
        if (req.userPermission !== 'secretary' || parseInt(req.userId) !== parseInt(secretary_id)) {
        ...
        let newSecretary
        try {
            newSecretary = await SecretaryModel.resetPassword(secretary_id, newPassword);
        } catch (err) {
